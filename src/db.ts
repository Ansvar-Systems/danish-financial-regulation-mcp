...
export function getDb(): Database.Database {
  if (_db) return _db;
  // Read-only: DB is baked into the image at build time (publish-ghcr.yml's
  // provisioning step downloads ftnet.db.gz from the latest GitHub Release
  // before docker build per the db_release_path manifest pattern). Runtime
  ...
  // file". Schema is exported for use by the offline ingestion scripts.
